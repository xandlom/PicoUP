...
fn sendUplinkPackets(state: *TestState, count: u32, phase: u8) !void {
    ...
    // Calculate UDP payload size (total - IP header - UDP header)
    const ip_udp_overhead: usize = 28; // 20 + 8
    const udp_payload_size = if (state.config.packet_size > ip_udp_overhead)
        state.config.packet_size - ip_udp_overhead
    else
        50; // Minimum payload

    var i: u32 = 0;
    while (i < count) : (i += 1) {
        // Create UDP payload of configured size
        var udp_payload: [1400]u8 = undefined;
        const payload_slice = udp_payload[0..udp_payload_size];
        ...
        const ip_packet_len = buildIpv4UdpPacket(
            ...
            ue_ip,
            external_dst_ip,
            12345 + @as(u16, @intCast(i % 100)), // Source port varies
            53, // DNS port
            payload_slice,
        );
    ...
    print("✓ Sent {} packets (~{} bytes total, UE {}.{}.{}.{} -> {}.{}.{}.{})\n", .{
        count,                                                                       total_bytes,
